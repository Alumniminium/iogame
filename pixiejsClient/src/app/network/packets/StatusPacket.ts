...
export class StatusPacket {
    ...
    constructor(header: PacketHeader, entityId: string, value: number, statusType: number) {
        this.header = header
    ...
    static handle(buffer: ArrayBuffer): void {
        ...
        if (!entity) {
            ...
            if (packet.statusType === 0 && packet.value === 1) {
                console.log(`Creating entity ${packet.entityId} from alive status packet`);
        ...
        switch (packet.statusType) {
            case 0: // Alive - handle despawn when value is 0
                if (packet.value === 0) {
                    ...
                    if (packet.entityId === localPlayerId) {
                        console.log(`⚠️ Server trying to destroy local player ${packet.entityId} - ignoring`);
                        return;
                    }
            ...
            case 2: // MaxHealth
                ...
                if (!maxHealth) {
                    maxHealth = new HealthComponent(entity.id, { max: 100, current: 100 });
