...
export class PacketHandler {
  private handlers = new Map<PacketId, (view: DataView, offset: number) => void>();
  
  registerHandler(id: PacketId, handler: (view: DataView, offset: number) => void): void {
  ...
  processPacket(data: ArrayBuffer): void {
    ...
    while (offset < data.byteLength) {
      ...
      if (handler) {
        ...
        console.warn(`Unknown packet ID: ${packetId} (length: ${packetLength})`);
      }
  ...
  static readString(view: DataView, offset: number): { value: string; bytesRead: number } {
    // Check if we have enough bytes to read the length
    if (offset + 2 > view.byteLength) {
      throw new RangeError(`Cannot read string length at offset ${offset}: buffer too small`);
    }
    ...
    if (offset + 2 + length > view.byteLength) {
      throw new RangeError(`Cannot read string of length ${length} at offset ${offset + 2}: buffer too small`);
    }
